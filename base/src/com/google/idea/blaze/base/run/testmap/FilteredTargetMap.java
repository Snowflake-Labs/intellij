...
public class FilteredTargetMap {
  ...
  private ImmutableSet<TargetIdeInfo> targetsForSourceFilesImpl(
    ...
    while (!todo.isEmpty()) {
      ...
      // Naming heuristic: Today we have two different kind of intermediate test libraries in snowflake codebase:
      // - `-test-lib` generated by BFG
      // - `.testlib` generated by `java_monolith_it_test` macro
      // These intermediate library may lose association with their test binary target due to macro implementation,
      // Here we use heuristic to locate the java_test binary rule of interest.
      ...
      TESTLIB_SUFFIXES.stream()
          .filter(targetName::endsWith)
          .findFirst()
          .ifPresent(suffix -> {
